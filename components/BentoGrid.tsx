...
const BentoCard: React.FC<{ project: Project }> = ({ project }) => {
  ...
  return (
    <div className={`group relative rounded-3xl overflow-hidden bg-surfaceHighlight border border-white/5 ${getSpanClass(project.span)}`}>
      ...
      {/* Content */}
      <div className="absolute inset-0 flex flex-col justify-end p-6 z-30 md:translate-y-8 md:group-hover:translate-y-0 transition-transform duration-300">
        <div className="flex flex-col">
          <div className="flex justify-between items-end">
            <div>
               <span className="inline-block px-2 py-1 mb-2 text-[10px] uppercase tracking-wider font-bold text-black bg-primary rounded-md md:opacity-0 md:group-hover:opacity-100 transition-opacity delay-75">
                 {project.category}
               </span>
               <h3 className="text-xl font-bold text-white drop-shadow-lg transform md:translate-y-2 md:group-hover:translate-y-0 transition-transform delay-75">
                 {project.title}
               </h3>
            </div>
            <div className="w-10 h-10 rounded-full bg-white text-black flex items-center justify-center md:opacity-0 md:group-hover:opacity-100 md:scale-50 md:group-hover:scale-100 transition-all duration-300 delay-100 shrink-0">
               <ArrowUpRight size={20} />
            </div>
          </div>
          
          <p className="text-sm text-neutral-300 mt-2 line-clamp-2 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 delay-100">
            {project.description}
          </p>
        </div>
      </div>
